...
describe("OneCurrency", () => {
    ...
    async function deployTokenFixture() {
        const [admin, minter, firstUser, secondUser] = await ethers.getSigners();
        ...
        return {
            token, admin, minter, firstUser, secondUser, MINTER_ROLE
        };
    }
    ...
    describe("minting", () => {
        it("Should allow MINTER_ROLE to mint tokens", async () => {
            const { token, minter, firstUser } = await loadFixture(deployTokenFixture);
    ...
    describe("Compliance (Blacklist)", () => {
        it("Should prevent blacklisted addresses from receiving tokens", async () => {
            const { token, admin, minter, firstUser } = await loadFixture(deployTokenFixture);
